...
def download_page(directory, link, id):
...
def save_cover(html_page, directory):
    ...
    return slugify(book_title), extension, cover_path

def read_chapters_and_titles(html_page):
...
def main(page_url):
    ...
    title, extension, cover_path = save_cover(html_page, directory)


    titles, chapters = read_chapters_and_titles(html_page)


    book = prepare_book(title=title, extension=extension, cover_path=cover_path)

    for i in range(len(chapters)):
        print(titles[i])

        add_chapter(book, titles[i], download_page(directory=directory, link=chapters[i], id=i), i)
...
url   = 'https://www.royalroad.com/fiction/49879/paranoid-mage'
